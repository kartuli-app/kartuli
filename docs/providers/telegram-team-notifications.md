...
- **CI failure notifications:** [notification-ci-failure.yml](https://github.com/kartuli-app/kartuli/blob/main/.github/workflows/notification-ci-failure.yml) — Two triggers: (1) **check_suite: completed** — when `conclusion == 'failure'` and the check suite is not from the GitHub Actions app (to avoid duplicate notifications; workflow failures are reported only by the workflow_run job), one job routes to Preview vs Production topic by `head_branch`. (2) **workflow_run** — when any of the three production workflows completes with failure on `main`, a second job sends to the CI Failures – Production topic (workflow name, run URL). Both jobs check out the repo before calling the Telegram action. **Note:** `check_suite` only triggers when the workflow file is on the default branch (main).
