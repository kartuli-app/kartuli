...
4. **`commit-diagrams`** (after `validate-all-monorepo` succeeds, same-repo PRs and `workflow_dispatch` only): checks out the **PR head branch** (`github.head_ref`) or `github.ref` on dispatch (not the default merge ref, so commits are not detached), sets `LEFTHOOK=0` so hooks do not run on the bot commit/push, installs Graphviz, runs **`pnpm run diagrams:all`** (delegates to [`@kartuli/diagram-generator`](../tools/diagram-generator/package.json); outputs under `diagrams/output/current/`), then commits that folder via [git-auto-commit-action](https://github.com/stefanzweifel/git-auto-commit-action) pinned by full commit SHA in the workflow, with `[skip ci]` in the message. Fork PRs skip this job. **Branch protection:** pushes from the default token do not re-run checks on the new bot SHA; if required checks must be green on the tip commit, use a PAT/App push, or do not require the orchestrator on that SHA, or avoid auto-commit on PRs.
