...
describe('parseAndMapLocalizedtData', () => {
  it('parses valid fake JSON and maps to LocalizedData with source on every item', () => {
    const result = parseAndMapLocalizedData(validFakeLocalizedJson, 'my-source');
    ...
    expect(result.localizedWordItems[0]).toEqual({
      id: 'word-x',
  ...
  it('throws when root is not an object', () => {
    expect(() => parseAndMapLocalizedData(null, 'src')).toThrow();
    expect(() => parseAndMapLocalizedData('string', 'src')).toThrow();
  });
});
